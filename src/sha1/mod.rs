...
impl Sha1 {
    ...
    pub fn update(&mut self, data: &[u8]) {
        ...
        //println!("starting update with state: {:x?}", self.state.state);
    ...
    pub fn update_with_state(&mut self, data: &[u8], a_state: [u32; 5]) {
        let len = &mut self.len;

        for (idx, s) in self.state.state.iter_mut().enumerate() {
            *s = a_state[idx];
        }

        //println!("starting update with state: {:x?}", self.state.state);
        let state = &mut self.state;
        //let mut state = Sha1State {state: a_state};
        self.blocks.input(data, |block| {
            *len += block.len() as u64;
            state.process(block);
        })
    ...
    /// Horrible way of getting the padding because I'm lazy/don't understand SHA well enough
    pub fn get_padding_bytes(data: &[u8]) -> Vec<u8> {
        let s = Sha1::from(data);
        
        return s.calc_padding();
    }

    fn calc_padding(&self) -> Vec<u8> {
        ...
        if blocklen < 56 {
            ...
            return last[blocklen..64].to_vec();
        } else {
            last[120..128].clone_from_slice(&extra);
            return last[blocklen..].to_vec();
        }
    }
    ...
    pub fn digest(&self) -> Digest {
        ...
        //println!("self.blocks.block at digest: {:x?}", self.blocks.block.to_vec());

        if blocklen < 56 {
            last[56..64].clone_from_slice(&extra);
            //println!("padding1 at digest: {:x?}", &last[blocklen..64]);
            state.process(as_block(&last[0..64]));
        } else {
            last[120..128].clone_from_slice(&extra);
            //println!("padding2 at digest: {:x?}", &last[blocklen..]);
            state.process(as_block(&last[0..64]));
            state.process(as_block(&last[64..128]));
        }

        //println!("state after digest:         {:x?}", self.state.state);
        //println!("local state after digest:   {:x?}", state.state);
        //println!("state after digest:   {:x?}", state.state);
...
impl Digest {
    ...
    pub fn u32s(&self) -> [u32; 5] {
        [self.data.state[0], self.data.state[1], self.data.state[2],
        self.data.state[3], self.data.state[4]]
    }
}
...
impl Sha1State {
    fn process(&mut self, block: &[u8; 64]) {
        ...
        //println!("processing {:x?}", block.to_vec());
        ...
        //println!("end of sha process. state is now : {:x?}", self.state);
    }
}
