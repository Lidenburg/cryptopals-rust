/// Stolen from https://rosettacode.org/wiki/MD4#Rust
...
pub fn md4<T: Into<Vec<u8>>>(input: T) -> [u32; 4] {
    ...
    for i in 0..n / 16 {
        ...
        //println!("Now working on: {:x?}", x);
        //println!("With state: {:x?} {:x?} {:x?} {:x?}", a, b, c, d);
        ...
        //println!("Done state: {:x?} {:x?} {:x?} {:x?}", a, b, c, d);
    }
...
pub fn md4_with_state<T: Into<Vec<u8>>>(input: T, state: [u32; 4], a_len: usize) -> [u32; 4] {
 
    let mut bytes = input.into().to_vec();
    let initial_bit_len = ((bytes.len() + a_len) << 3) as u64;
    ...
    //println!("With state: {:x?}", state);
    //println!("Now hashing2: {:x?}", bytes);
    bytes.push(0x80_u8);
    while (bytes.len() % 64) != 56 {
        bytes.push(0_u8);
    }
    ...
    //println!("initial_bit_len with state: 0x{:x}", initial_bit_len);
    //println!("after padding: {:x?}", w);
    //println!("after padding: {:x?}", w);
...
pub fn get_padding_bytes<T: Into<Vec<u8>>>(input: T) -> Vec<u8> {
    let mut bytes = input.into().to_vec();
    let initial_bit_len = (bytes.len() << 3) as u64;
    ...
    let byte_offset = start_pos % 4;

    // There we go
    for i in byte_offset..4{
        resvec.push(((w[start_pos / 4] >> (i * 8)) & 0xffu32) as u8);
    }

    for b in &w[(start_pos/4)+1..] {
        resvec.push(((b >> (0 * 8)) & 0xffu32) as u8);
        resvec.push(((b >> (1 * 8)) & 0xffu32) as u8);
        resvec.push(((b >> (2 * 8)) & 0xffu32) as u8);
        resvec.push(((b >> (3 * 8)) & 0xffu32) as u8);
    ...
    return resvec;
    //return bytes[start_pos..].to_vec();
}
 
pub fn digest_to_str(digest: &[u32]) -> String {
